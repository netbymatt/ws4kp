...
import applyScanlineScaling from './scanlines.mjs';
...
// the scale actually applied to the display, used by the scan lines and reported in the headend
let appliedScale = 1;
...
const init = () => {
	...
	// fired by settings.mjs when scan lines are switched on or their style changes
...
const resize = (force = false) => {
	...
	// the scale actually applied to the display, for the scan lines and the headend (the branch below that doesn't scale sets 1)
	appliedScale = scale;
	...
	if (!isKioskLike && scale >= 1.0 && !isKioskMode) {
		...
		// no scaling is applied here, so the scan lines and the headend use 1x
		appliedScale = 1.0;
...
const displayScale = () => appliedScale;
